...
`openai-codex-oauth.e2e.ts` mounts the scaffold's deterministic Codex device-code controller around the real pi-ai catalog. Its completion barrier first emits the redacted OAuth connection event while the route is still absent, lets the browser prove that event refreshed the Models card, and only then registers the route for the normal Host model picker, default, and per-session selection checks. A real model request then reaches a deterministic native-provider failure carrying a hostile sentinel; the scenario proves that stream events, durable persistence, browser history and event payloads, Host and browser logs, visible copy, and the committed golden contain only the stable provider-neutral failure. The scenario is keyless and its only device code is `ABCD-EFGH` at a fixed verification URL.
