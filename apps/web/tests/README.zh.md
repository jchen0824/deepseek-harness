...
`openai-codex-oauth.e2e.ts` 将 scaffold 的确定性 Codex 设备码控制器挂在真实 pi-ai 模型目录外。它的完成屏障会在路由仍不存在时先发出经过删减的 OAuth 连接事件，让浏览器证明该事件刷新了“模型”卡片，然后才注册路由，供普通 Host 模型选择器、默认值和会话级选择检查使用。随后，一次真实模型请求会抵达携带恶意哨兵值的确定性原生提供方失败；该场景证明流事件、持久化存储、浏览器历史与事件载荷、Host 与浏览器日志、可见文案及已提交快照都只包含稳定且与提供方无关的失败。该场景不需要密钥，唯一设备码是固定验证网址上的 `ABCD-EFGH`。
