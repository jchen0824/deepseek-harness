...
`llm.providers` 包含每个提供方明确的认证方式，但绝不包含 OAuth 连接状态。`llm.oauthStart`、`llm.oauthStatus`、`llm.oauthCancel` 与 `llm.oauthDisconnect` 只接受目录中已配置、认证方式为 OAuth 且控制器已注册的条目；任意提供方 id 与非 OAuth 路由得到相同的不可用答复。start 还可能直接返回一项设备代码操作，其中仅有 `verificationUri`、`userCode` 与可选计时字段；其余三种答复只包含连接视图。生命周期答复都不会携带 access token 或 refresh token、账户或计划数据、凭据引用、租约或提供方原始错误，控制器失败则转换为稳定、可操作的消息。owner 已脱敏的 `llm/oauth-connection-updated` 事件仍仅在 Host 内部使用，不在转发事件名单中。浏览器载体把模型发现与全部四个 OAuth 方法固定在回环地址；status 与写操作采用同一策略，避免匿名 LAN 调用方探测已配置的连接。
