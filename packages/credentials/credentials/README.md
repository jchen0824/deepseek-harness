...
`credentials/updated (ref)` fires after a committed public change to a provider-managed source — a `set`, an `unset`, or an external edit observed in storage. Ambient process-environment changes are not observable and never emit. Consumers do not need the event (they re-resolve per operation); it exists for configuration UIs refreshing a "configured" badge. A private mutation instead emits payload-free `credentials/private-updated` for Host consumers to refresh private state; it carries no reference or value and is never forwarded to browser clients. Its public declaration lives in the client-safe `./types` subpath export together with the `CredentialRef` type it names (the package root re-exports the type), so a consumer outside the Host compilation face reads the very signature the Host emits instead of restating it.
