...
`credentials/updated (ref)` 在提供方管理的来源发生已提交的公开变更后触发——`set`、`unset` 或在存储中观察到的外部编辑。进程环境变量的变化不可观测，永不触发。消费方不需要该事件（它们按操作重新解析）；它服务于配置界面刷新「已配置」徽标。私有修改则发出不带 payload 的 `credentials/private-updated`，供 Host 消费方刷新私有状态；它不携带引用或值，也绝不转发给浏览器客户端。公开事件的声明住在 client-safe 的 `./types` 子路径出口，与其点名的 `CredentialRef` 类型同处一处（包根继续 re-export 该类型），于是 Host 编译面之外的消费方读到的正是 Host 发射的那一份签名，而不必再写一遍。
