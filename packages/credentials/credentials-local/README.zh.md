...
外部编辑在快照**整体替换**后按变更的公开引用逐个发布 `credentials/updated`——磁盘上删掉的条目绝不在内存滞留。变更的私有引用改为产生一次不带 payload、仅供 Host 使用的 `credentials/private-updated` 失效通知，使另一进程能够刷新私有状态而不获知引用。在 Chokidar 打开目标之前，提供方会对层级最深的现有祖先路径执行 realpath 解析，再拼回缺失的后缀；文件访问和诊断仍使用配置路径，从而避免 Windows 混用 8.3 别名与 libuv 的长格式事件路径。提供方自己的写入按内容识别，只发布与该次提交相符的一次失效通知。运行期文档不可读或无效时保留最后可用快照并告警；文件不存在即空存储；启动时不可读或无效则明确报错。
