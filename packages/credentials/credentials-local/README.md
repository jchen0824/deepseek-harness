...
External edits publish `credentials/updated` per changed public reference after the snapshot is replaced **wholesale** — an entry deleted on disk never lingers in memory. Changed private references instead produce one payload-free host-only `credentials/private-updated` invalidation, so another process can refresh private state without learning a reference. Before Chokidar opens the target, the provider realpaths its deepest existing ancestor and restores any missing suffix; file access and diagnostics retain the configured path, while Windows cannot mix an 8.3 alias with long-form libuv events. The provider's own writes are recognized by content and publish exactly their one matching invalidation. An unreadable or invalid document at runtime keeps the last good snapshot and warns; an absent file is an empty store; an unreadable or invalid file at boot fails loud.
