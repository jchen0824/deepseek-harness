...
profile 未指定 `apiKeyEnv` 的 OAuth 行使用专用生命周期方法，绝不走通用 API 密钥编辑器。已经存在且显式指定 `apiKeyEnv` 的 Codex profile 保持密钥认证：它显示为普通密钥行，绝不会提供 OAuth 生命周期操作，并遵循普通凭据/profile 删除。原生 OAuth 删除会先请求断开连接，并且只有控制器返回终态 `missing` 后才移除 profile；仍为 connecting 或 reconnect-required 的答复会保留已配置行并显示操作失败，使用户可以重试。
...
普通提供方配置会通过这些失效通知收敛，无需轮询。处于 connecting 的 OAuth 卡片是例外：它会轮询仅限回环地址的状态直至终态，因此设备码过期或同级进程取消时，显示状态不会在不广播生命周期状态的情况下停留为过时值。
