...
该特权集合中的 LLM 成员是 `llm.discoverModels` 与完整的 OAuth 方法族：`llm.oauthStart`、`llm.oauthStatus`、`llm.oauthCancel` 和 `llm.oauthDisconnect`。模型发现接收一份草拟凭据，并让 Host 获取调用方选定的 URL；OAuth 写操作会改变持久连接状态，而 status 采用同一策略，避免匿名 LAN 调用方探测已配置的 OAuth 连接。`llm.providers` 与 `llm.models` 仍可供已声明的权威访问，因为模型选择器需要其中的脱敏目录数据。
