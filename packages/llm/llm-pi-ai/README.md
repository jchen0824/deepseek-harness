...
Disconnect advances the generation, clears the stored OAuth credential, and calls pi-ai logout through that revoked generation's store, so a stale logout cannot remove a later connection. Each native OAuth request captures its generation before authentication; a failed refresh marks reconnect-required only when that generation remains current. A revoked credential, failed refresh, unreadable record, or missing credential provider becomes the redacted `reconnect-required` lifecycle and `OAUTH_RECONNECT_REQUIRED` request failure; a native OAuth profile never falls through to an API key, process environment, or another provider. Every other native Codex provider error is classified internally but reaches stream chunks and thrown failures only as `OpenAI Codex request failed`, so no provider-owned message can enter a session, history response, log, or snapshot. An existing profile that explicitly names `apiKeyEnv` remains a separate legacy key-auth route and does not become the fallback for OAuth.
