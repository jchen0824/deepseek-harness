...
`openai-codex` is always present in the configurable-provider directory with OAuth authentication metadata. When its profile exists, the adapter registers the ordinary pi-ai catalog route regardless of private OAuth state, so public provider and model topology cannot disclose whether a subscription is connected. The Web Models page creates the empty profile and starts the connection; its direct loopback status decides whether the route is usable. A disconnected native OAuth request fails as `OAUTH_RECONNECT_REQUIRED`. The normal model picker, defaults, per-session selection, reasoning metadata, and request path remain authoritative. OpenAI controls which catalog models the connected subscription may use; catalog membership is not an entitlement check.

The controller accepts only pi-ai's OpenAI Codex device-code selector. The initiating Host call receives the verification URL and one-time code; browser-callback, manual-code, secret, pasted-token, and other prompts fail with the package's redacted OAuth error. Device-code data lives only for that initiating interaction and is cleared on settlement or cancellation. Host-internal connection broadcasts contain only `provider` and `status`; public provider and model catalogs and forwarded browser events do not include connection state.

One host-lifetime `OpenAICodexCredentialStore` is shared by the controller and every immutable pi-ai model collection. It stores a versioned OAuth record with a monotonic generation through `CredentialProvider.modify()` as a private atomic mutation, so refreshes from different model snapshots and Harness processes serialize on the local credential provider's cross-process lock. Beginning a login generation clears any credential from earlier authority, and its generation-scoped store rejects writes after revocation; a cancellation racing a replacement login therefore cannot relabel an old credential as the replacement connection. The fixed private credential reference never appears in plugin configuration, provider profiles, settings, events, logs, or browser data; private changes emit only the payload-free host-private credential invalidation that makes peer controllers re-read their status.

Login uses a second fixed private record for the generation and an expiring cross-process lease. Exactly one process claims and renews it while pi-ai polls; another process reports `already-connecting`, and an expired lease can be taken over. Each peer that observes a live lease schedules a local re-read at expiry, so a credential persisted just before an owner crash converges connection state without another file change. Cancel or disconnect advances the durable generation and clears the lease, so a remote poller cannot persist credentials after revocation; orderly plugin disposal aborts owned work and releases only its matching lease. The same record carries `reconnect-required`, making refresh failure visible to every process sharing the Harness home. `oauth.loginLeaseTtlMs` is a positive bounded top-level plugin setting, defaults to 30 seconds, and supplies both expiry and renewal timing:
...
Disconnect advances the generation, clears the stored OAuth credential, and calls pi-ai logout through that revoked generation's store, so a stale logout cannot remove a later connection. Each native OAuth request captures its generation before authentication; caller cancellation also covers that capture and pi-ai's `getAuth()` preflight, so an already-cancelled request starts neither. pi-ai cannot externally cancel an in-flight network refresh, but the request stops waiting and its abort-aware credential store rejects the durable commit, so a late refresh cannot persist a rotated credential. A failed refresh marks reconnect-required only when that generation remains current and its private coordination mutation commits. If the marker cannot commit, the request still fails as `OAUTH_RECONNECT_REQUIRED` and leaves the published connection state unchanged. A revoked credential, durably marked failed refresh, unreadable record, or missing credential provider becomes the redacted `reconnect-required` lifecycle and `OAUTH_RECONNECT_REQUIRED` request failure; a native OAuth profile never falls through to an API key, process environment, or another provider. Harness-owned `LlmError` failures retain their stable code and message. Native Codex provider and SDK errors are classified internally but reach stream chunks and thrown failures only as `OpenAI Codex request failed`, so no provider-owned message can enter a session, history response, log, or snapshot. An existing profile that explicitly names `apiKeyEnv` remains a separate legacy key-auth route, does not become the fallback for OAuth, and rejects OAuth lifecycle RPCs.

The Web and Headless profiles share the connection when they use the same Harness home. Headless never starts interactive login: it consumes an already-connected profile and the same credential store; a disconnected native OAuth request returns `OAUTH_RECONNECT_REQUIRED`. The implementation-owned OAuth record and lease references are never fields a user copies into `settings.yaml` or `cordis.yml`.
