...
- `ctx.llm.registerConfigurableProviders(entries: readonly LlmConfigurableProvider[]): DirectoryRegistrationHandle` 声明适配器插件可通过配置激活的提供方路由——无论已注册还是休眠——每个条目指明其所属 settings namespace、profile 在该分节内的路径以及必需的提供方无关认证方式。要么全部成功，要么全部不生效（`INVALID_DIRECTORY`/`DUPLICATE_DIRECTORY`），并随调用 fiber dispose。该句柄还带 `replace(entries)`：候选集合会先被整体校验，因此其中若有条目已被另一个注册声明，当前集合原封不动；此处允许传空数组。声明集合随配置变化的插件必须使用 `replace`，而不是先 dispose 再重新注册——后者会在新集合被拒时让目录整个落空。
...
- `ctx.llm.registerOAuthController(controller: LlmOAuthController): () => void` 注册一个由 fiber 拥有的 OAuth 生命周期 controller。公开读取获得的是绑定提供方的 façade，而不是传入的 controller；每个状态、取消、断连和启动结果都会重建为脱离原对象的脱敏数据。
- `ctx.llm.getOAuthController(provider: string): LlmOAuthController | undefined` 读取已注册的、绑定提供方的 OAuth façade。
- `ctx.llm.emitOAuthConnectionUpdated(connection: LlmOAuthConnection): void` controller 提交生命周期变化后广播脱敏的 `{ provider, status }` 更新。设备码、凭据、帐户数据和提供方错误绝不会进入该事件；损坏的 observer 会被隔离。
