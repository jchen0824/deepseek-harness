...
- `ctx.llm.registerOAuthController(controller: LlmOAuthController): () => void` Register one fiber-owned OAuth lifecycle controller. Public reads receive a provider-bound façade, never the supplied controller; every status, cancellation, disconnection, and start result is reconstructed as detached redacted data.
- `ctx.llm.getOAuthController(provider: string): LlmOAuthController | undefined` Read the registered provider-bound OAuth façade.
- `ctx.llm.emitOAuthConnectionUpdated(connection: LlmOAuthConnection): void` Broadcast a redacted `{ provider, status }` update after a controller commits a lifecycle change. Device codes, credentials, account data, and provider errors never enter this event; broken observers are contained.
